...
def main():
    ...
    parser.add_argument("--injected-dir", type=str, default=None, help="Positive class dir (default: <script_dir>/injected_correct)")
    parser.add_argument("--clean-dir", type=str, default=None, help="Negative class dir (default: <script_dir>/not_detected)")
    ...
    # Load positive class (detected / injected_correct)
    for concept_dir in [d for d in injected_dir.iterdir() if d.is_dir()]:
        for f in concept_dir.glob(f"*_layer{layer}_coeff{coeff}_*.pt"):
            try:
                vec = torch.load(f, map_location="cpu")['activations']['last_token']
                X_list.append(vec.float().numpy())
                y_list.append(1)  # DETECTED
            except Exception:
                continue

    # Load negative class (not_detected)
    for concept_dir in [d for d in clean_dir.iterdir() if d.is_dir()]:
        for f in concept_dir.glob(f"*_layer{layer}_coeff{coeff}_*.pt"):
            try:
                vec = torch.load(f, map_location="cpu")['activations']['last_token']
                X_list.append(vec.float().numpy())
                y_list.append(0)  # NOT DETECTED
            except Exception:
                continue

    if not X_list:
        print(f"ERROR: No data found! Check that .pt files exist with layer={layer}, coeff={coeff}")
        return

    X = np.stack(X_list)
    y = np.array(y_list)
    print(f"Loaded {len(X)} examples. (Positive/Detected: {sum(y)}, Negative/NotDetected: {len(y)-sum(y)})")

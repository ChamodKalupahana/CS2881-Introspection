...
def _query_yes_no(prompt_template: str, prompt_text: str, response: str) -> bool:
    """Query the LLM judge with a YES/NO prompt. Returns True or False."""
...
def classify_response(response: str, concept: str, messages: list[dict]) -> str:
    """
    Classify the model's response using a 3-stage LLM judge pipeline:
    'incoherent', 'not_detected', 'detected_opposite', 'detected_orthogonal', 
    ...
    # ── Stage 2: Quick check - did the model affirm detection? ───────────
    ...
    # ── Stage 3: Detailed classification ─────────────────────────────────
    ...
    try:
        ...
        if category:
            # Stem upgrade: if 3+ char stem of concept appears in response, likely correct
            resp_lower = response.lower()
            concept_lower = concept.lower()
            stem = concept_lower[:max(3, len(concept_lower) - 3)]
            if category in ("detected_parallel", "detected_orthogonal") and stem in resp_lower and len(stem) >= 4:
...
def inject_and_capture_activations(
    ...
    return response, activations, messages
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default=str(PROJECT_ROOT / "linear_probe" / "specificity" / "saved_activations"))
        ...
        for layer in args.layers:
            ...
            for concept, (vec_last, vec_avg) in vectors.items():
                ...
                for coeff in args.coeffs:
                    ...
                    try:
                        response, activations, messages = inject_and_capture_activations(

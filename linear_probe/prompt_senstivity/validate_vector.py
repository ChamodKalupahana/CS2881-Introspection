...
def main():
    ...
    if args.run_dir:
        ...
        if not candidates:
            print("ERROR: No prompt_activations_* directories found.")
            return
    ...
    pos_vecs, _, _ = load_class_vectors(pos_dir, layer, token_type, allowed_prompts)
    neg_vecs, _, _ = load_class_vectors(neg_dir, layer, token_type, allowed_prompts)
    opp_vecs, _, _ = load_class_vectors(pos_root / "detected_opposite", layer, token_type, allowed_prompts)
    ortho_vecs, _, _ = load_class_vectors(pos_root / "detected_orthogonal", layer, token_type, allowed_prompts)
    para_vecs, _, _ = load_class_vectors(pos_root / "detected_parallel", layer, token_type, allowed_prompts)
    notdet_vecs, _, _ = load_class_vectors(pos_root / "not_detected", layer, token_type, allowed_prompts)
    incoh_vecs, _, _ = load_class_vectors(pos_root / "incoherent", layer, token_type, allowed_prompts)

    if not pos_vecs or not neg_vecs:
        print(f"ERROR: Missing primary class data! Pos: {len(pos_vecs)}, Neg: {len(neg_vecs)}")
        return
    ...
    plt.title(f"1D Projection onto Input Vector: {vec_name} (Layer {layer}, {token_type})")

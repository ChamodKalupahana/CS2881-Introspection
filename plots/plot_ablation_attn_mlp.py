"""Plot ablation or patch results: attn vs mlp across layers and concepts.

Reads all CSVs from a results directory (auto-detects ablate or patch mode).
Generates:
  1. Multi-concept heatmap (layers × sublayer → category)
  2. Detection survival rate (line plot)
  3. Per-concept stacked bars

Usage:
    python plots/plot_ablation_attn_mlp.py
    python plots/plot_ablation_attn_mlp.py --input_dir acitvation_patching/results/patch
    python plots/plot_ablation_attn_mlp.py --input_dir acitvation_patching/results/ablate
"""
...
# Numeric score: how much "detection" survived the ablation
CAT_SCORE = {
...
def parse_csv(csv_path):
    """Parse a patching/ablation CSV. Supports both formats:
    - New: patch_target, detection_category, raw_response_prefix
    - Old: patch_target, detected_correct, ..., not_detected, raw_response_prefix
    """
    rows = []
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            ...
            # New format: single detection_category column
            if "detection_category" in row:
                cat = row["detection_category"].strip()
                if cat not in CATEGORIES:
                    cat = "not_detected"
            else:
                # Old format: one-hot columns
                cat = "not_detected"
                for c in CATEGORIES:
...
def extract_concept_from_filename(filename):
    """Extract concept name from various filename patterns."""
    ...
    if m:
        return m.group(1)
    # Pattern: patch_attn_mlp_results_02_13_26_21_41_Frostbite.csv
    m = re.search(r"results_[\d_]+_(.+?)\.csv", filename)
    if m:
        return m.group(1)
    return filename
...
def load_all_csvs(input_dir):
    """Load all ablation/patch CSVs from directory. Returns dict: concept -> rows."""
    data = {}
...
def plot_multi_concept_heatmap(data, save_dir, mode="ablate"):
    """
    ...
    ax.set_title(f"{mode_title} Results: Detection Category per Layer × Sublayer",
...
def plot_detection_survival(data, save_dir, mode="ablate"):
    """
    Line plot: detection score across layers, separate lines for attn/mlp.
    ...
    for ci, concept in enumerate(concepts):
        ...
        ax.plot(layers, attn_scores, 'o-', color="#3498db", linewidth=2,
                markersize=6, label="attn ablated", alpha=0.9)
        ax.plot(layers, mlp_scores, 's-', color="#e67e22", linewidth=2,
                markersize=6, label="mlp ablated", alpha=0.9)
    ...
    fig.suptitle(f"Detection Survival After {mode_label} (attn vs mlp)",
...
def plot_stacked_bars(data, save_dir, mode="ablate"):
    """
    ...
    mode_label = "Ablation" if mode == "ablate" else "Patch"
    fig.suptitle(f"{mode_label}: Category Breakdown by Layer (A=attn, M=mlp)",
...
def main():
    ...
    parser.add_argument("--input_dir", type=str,
                        default=str(PROJECT_ROOT / "acitvation_patching" / "results" / "ablate"))
